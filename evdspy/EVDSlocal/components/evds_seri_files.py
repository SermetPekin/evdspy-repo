...
class EvdsSeriesRequest:
    ...
    def update_url_instance(self):
        ...
        ds: DateStart = DateStart(value=self.start_date)
        de: DateEnd = DateEnd(value=self.end_date)
        slist: Tuple[str] = tuple(x.ID for x in self.series_list)
        series_instance = Series(slist)
        full_list = [series_instance, ds, de]
        complete_url_instance = URLClass(full_list)
        # assert len(complete_url_instance.url_items) > 0, "url_items"
        # self.complete_url_instance = complete_url_instance
